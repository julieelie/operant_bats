...
% 'SaveMode': set to 'mat' for saving all vocalizations in a mat file or
...
% Get input arguments
Pnames = {'TransceiverTime', 'SaveMode'};
TranscTime = 1; % Logical to indicate if transceiver time should be calculated for these extracts.
Dflts  = {TranscTime, 'wav'};
[TranscTime, SaveMode] = internal.stats.parseArgs(Pnames,Dflts,varargin{:});
...
% We choose to have 0.5s of recording before and after the vocalization
% onset/offset
% to better isolate the vocalization
Buffer_cut = 0.5;
Fhigh_power =20; % Frequency upper bound for calculating the envelope (time running RMS)
Fs_env = 1000; % Sample freqency of the enveloppe Hz
AmpThreshPerc = 10/100; % Thershold of amplitude used to cut the extract around the vocalization (10% max)
...
for ss=1:length(FullStamps)
    ...
    if ~Done
        ...
        WavFileStruc_local = dir(fullfile(RawWav_dir, sprintf('%s*mic*_%d.wav',DataFile(1:16), File_Idx)));
        try
        ...
        % the snip folder)
        DataSnipStruc = dir(fullfile(AudioDataPath, sprintf('%s_%s_%s*snippets/*snipfile_%d_%d.wav', Subj, Date, Time, yy,Stamp_or)));
        ...
        if NbSnip_local
            ...
            fprintf('No snippets of sound saved, the vocalization cutting will be done without a better approximation of the sound localization\n')
        end
        ...
        OnsetSamp_Ysection = OnsetSamp/Fs_env*FS - Buffer_cut*FS;
        OffsetSamp_Ysection = OffsetSamp/Fs_env*FS - Buffer_cut*FS;
        ...
        hold on
        plot([OnsetSamp_Ysection OffsetSamp_Ysection], [0 0], 'r')
        hold off
        title('Tiggered vocalization %d/%d', ss, length(FullStamps));
        ...
        if TranscTime
            ...
            Voc_transc_times(ss,:) = TTL.Mean_std_Pulse_TimeStamp_Transc(TTL_idx,2) .* polyval(TTL.Slope_and_intercept{TTL_idx},Voc_samp_idx_zs,[], TTL.Mean_std_x{TTL_idx}) + TTL.Mean_std_Pulse_TimeStamp_Transc(TTL_idx,1);
        end
    end
end

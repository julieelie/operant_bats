...
if ~exist(ExpLog, 'file')
    Fid = fopen(ExpLog, 'a');
    fprintf(Fid, 'BatID\tDate\tTime\tHigh-Pass(Hz)\tLow-Pass(Hz)\tNumVocs\n');
    DoneList = [[], [], []];
else
    Fid = fopen(ExpLog, 'r');
...
for bb=1:length(BoxOfInterest) % for each box
    ...
    for ff=1:length(ParamFilesDir)
        ...
        if ~isempty(DataFileStruc) > 0
            ...
            if ~isempty(IndexLineLow) && ~isempty(IndexLineHigh) 
            ...
            try
                ...
                fprintf(Fid, '%s\t%s\t%s\t%f\t%f\t%d\n',ParamFilesDir(ff).name(1:4),ParamFilesDir(ff).name(6:11),ParamFilesDir(ff).name(13:16),high,low,numVocs);
            catch ME

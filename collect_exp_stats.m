...
%         numVoc = vocID.length;
        ...
        if toDo ~= 0
            ...
            try
                ...
                fprintf(Fid, '%s\t%s\t%s\t%s\t%f\t%f\t%d\n',ParamFilesDir(ff).name(1:4),ParamFilesDir(ff).name(6:11),ParamFilesDir(ff).name(13:16),ParamFilesDir(ff).name(18:(Ind_-1)),high,low,LoggerDataYN);
                fprintf(1, '%s\t%s\t%s\t%s\t%f\t%f\t%d\n',ParamFilesDir(ff).name(1:4),ParamFilesDir(ff).name(6:11),ParamFilesDir(ff).name(13:16),ParamFilesDir(ff).name(18:(Ind_-1)),high,low,LoggerDataYN);
                ME

...
for ss=1:NVoc
    ...
    if ~Done
        ...
        while isempty(PKs) % try with a lower threshold
            [PKs,~] = findpeaks(Amp_env_Mic,Fs_env, 'MinPeakHeight', InitialAmpFact*MeanStdAmpRawFile(File_Idx,1));% detect all the peaks in the amplitude envelope that are higher than twice the mean of the enveloppe
